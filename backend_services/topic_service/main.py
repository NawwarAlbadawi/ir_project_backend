"""
=============================================================================
 Topic Service — main.py
 Port: 8006
=============================================================================
 Discovers latent topics in a corpus using LDA and exposes endpoints to:
   - Build a topic model for a dataset
   - Get the dominant topic of one or many documents
   - Infer the topic of a new query string
=============================================================================
"""
...
from topic_service.schemas import (
    BuildTopicsRequest, BuildTopicsResponse,
    DocTopicRequest, DocTopicResponse, DocTopicResult,
    TopicDefinition, TopicStatusResponse,
)
...
# ---------------------------------------------------------------------------
# Lifespan — restore cached models on startup
# ---------------------------------------------------------------------------
@asynccontextmanager
...
app = FastAPI(
    ...
    description="LDA-based topic modeling. Discovers latent topics in the corpus and assigns each document a topic.",
    version="1.0.0",
    lifespan=lifespan,
)
...
def build_topics(request: BuildTopicsRequest, background_tasks: BackgroundTasks):
    """
    Train an LDA topic model for a dataset.

    Fetches preprocessed documents from the Preprocessing Service,
    trains LDA, and saves the model to disk. Runs in background.
    Use GET /topics/status to poll progress.
    """
...
def topic_status(dataset_name: str = Query(...)):
    """Check whether the topic model has been built for a dataset."""
    state = _get_or_create(dataset_name)
    with state._lock:
        return TopicStatusResponse(
            dataset=dataset_name,
            status=state.status,
            num_topics=state.model.num_topics,
            error=state.error,
        )
...
def get_all_topics(dataset_name: str = Query(...)):
    """Return all topic definitions (id, label, top words, doc count)."""
    state = _get_or_create(dataset_name)
...
# ===========================================================================
# Detect topics for documents
# ===========================================================================
@app.post("/topics/detect", response_model=DocTopicResponse, tags=["topics"])
def detect_topics(request: DocTopicRequest):
    """
    Return the dominant topic for each document ID in the list.

    Typically called after a search to enrich results with topic labels.
    """
    state = _get_or_create(request.dataset_name)
    if state.status != "ready":
        raise HTTPException(
            404,
            f"Topic model not built for '{request.dataset_name}'. "
            "Call POST /topics/build first."
        )
    ...
    for doc_id in request.doc_ids:
        ...
        if info:
            ...
            # Doc not in model (e.g. added after training) — return unknown
            results.append(DocTopicResult(
                doc_id=doc_id,
                topic_id=-1,
                topic_label="unknown",
...
def infer_query_topic(
    ...
    text: str = Query(..., description="Preprocessed query text to infer topic for"),
):
    """Infer the topic of a new text (e.g. a search query)."""
    state = _get_or_create(dataset_name)
    if state.status != "ready":
        raise HTTPException(404, f"Topic model not ready for '{dataset_name}'.")
    try:
...
def _build_pipeline(state: _TopicState, request: BuildTopicsRequest) -> None:
    ...
    try:
        # Fetch all preprocessed docs from Preprocessing Service
        ...
        state.model.build(
            documents=docs,
            num_topics=request.num_topics,
            num_top_words=request.num_top_words,
        )
...
def _fetch_all_documents(dataset_name: str) -> list[dict]:
    ...
    with httpx.Client(timeout=120.0) as client:
        while True:
            resp = client.get(
                f"{PREPROCESSING_URL}/dataset/docs",
