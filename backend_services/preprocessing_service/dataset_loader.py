"""
=============================================================================
 Preprocessing Service — dataset_loader.py
=============================================================================
 Loads, preprocesses, and caches datasets from `ir_datasets`.

 Supported datasets (by short name → ir_datasets key):
   "quora"   → "beir/quora/test"
   "msmarco" → "msmarco-passage/train/triples-small"  (sampled to 200K docs)

 Caching strategy:
   Raw datasets are cached by ir_datasets in its own cache directory.
   Our preprocessed outputs are saved as JSON files under /data/<name>/
   so the service can restart without re-processing the whole corpus.
=============================================================================
"""
...
# Configuration
# ---------------------------------------------------------------------------
# Base directory for our own preprocessed cache files.
# Goes to /data/<dataset_name>/ relative to this file's parent-parent.
_BASE_DIR = Path(__file__).resolve().parent.parent.parent / "data"

# Mapping of friendly short name → ir_datasets canonical name
DATASET_REGISTRY: dict[str, str] = {
    "quora": "beir/quora/test",
    "msmarco": "beir/msmarco/test",   # BEIR version → has proper test qrels
}

# For msmarco we subsample to keep things tractable on a standard machine
MSMARCO_MAX_DOCS = 200_000

# Preprocessing options used when building the cached corpus.
# Keep in sync with what the Indexing Service expects.
DEFAULT_PREPROCESS_OPTS = PreprocessOptions(
    lowercase=True,
    remove_punctuation=True,
    remove_stopwords=True,
    stem=True,
    lemmatize=False,  # Stemming already active; lemmatize adds latency
)
...
# ---------------------------------------------------------------------------
class _DatasetState:
    """Mutable state for one dataset (thread-safe via a lock)."""
    ...
    def __init__(self, name: str):
        ...
        # Loaded data (populated once "ready")
        self.raw_docs: dict[str, str] = {}        # doc_id → raw text
        self.preprocessed_docs: list[dict] = []   # [{doc_id, tokens, cleaned}]
        self.doc_ids: list[str] = []              # ordered list of doc_ids
        self.queries: list[dict] = []             # [{query_id, original, tokens, cleaned}]
        self.qrels: dict[str, dict[str, int]] = {}  # {query_id: {doc_id: rel}}
    ...
    def to_status_response(self) -> DatasetStatusResponse:
        with self._lock:
            return DatasetStatusResponse(
                dataset=self.name,
                status=self.status,
                progress_docs=self.progress_docs,
                total_docs=self.total_docs,
                progress_queries=self.progress_queries,
...
class DatasetLoader:
    """
    Manages loading, preprocessing, and disk-caching of IR datasets.
    ...
    def get_documents(
        self, dataset_name: str, offset: int = 0, limit: int = 100
    ) -> Optional[list[dict]]:
    ...
    # ------------------------------------------------------------------
    # Loading pipeline (runs in a background thread)
    # ------------------------------------------------------------------
    def load_dataset(self, dataset_name: str) -> None:
        """
        Full pipeline:
          1. Try to restore from disk cache.
          2. If not cached, download/iterate via ir_datasets and preprocess.
    ...
    def _load_documents(
        ...
        """Iterate the corpus, preprocess, collect raw text, save cache."""
        ...
        for doc in dataset.docs_iter():
            ...
            # Different datasets expose text under different attribute names
            raw_text = getattr(doc, "text", "") or getattr(doc, "body", "") or ""

            tokens = self._preprocessor.preprocess(raw_text, options=opts)
            preprocessed.append(
                {"doc_id": doc_id, "tokens": tokens, "cleaned": " ".join(tokens)}
            )
            raw_docs[doc_id] = raw_text
            doc_ids.append(doc_id)
            count += 1

            # Update progress every 5000 docs
            if count % 5_000 == 0:
                with state._lock:
        ...
        logger.info(
            f"[{dataset_name}] Preprocessed {count} docs in {elapsed:.1f}s "
            f"({count / max(elapsed, 1):.0f} docs/s)"
        )
    ...
    def _try_load_from_cache(self, state: _DatasetState, cache_dir: Path) -> bool:
        """
        Attempt to restore a fully-preprocessed dataset from disk.
