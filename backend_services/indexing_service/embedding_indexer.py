"""
=============================================================================
 Indexing Service — embedding_indexer.py
=============================================================================
 Builds dense-vector indexes using:
   a) Word2Vec (gensim) — average word vectors per document
   b) BERT sentence embeddings (sentence-transformers: all-MiniLM-L6-v2)

 Both produce an (N × D) float32 numpy matrix where:
   - N = number of documents
   - D = embedding dimension (Word2Vec: configurable, MiniLM: 384)

 Similarity search uses cosine similarity via:
   numpy dot product on L2-normalised vectors → O(N·D) per query.
   For production you would replace this with FAISS or HNSW.
=============================================================================
"""
...
# Lazy imports to avoid import-time delays when the service starts
# and to let us handle missing optional dependencies gracefully.
...
# ---------------------------------------------------------------------------
# Helper: cosine similarity between a query vector and a matrix
# ---------------------------------------------------------------------------
def _cosine_scores(query_vec: np.ndarray, matrix: np.ndarray) -> np.ndarray:
    ...
    Both query_vec and matrix rows must be L2-normalised for this to be
    equivalent to a dot product.
    """
    q_norm = query_vec / (np.linalg.norm(query_vec) + 1e-10)
    # matrix rows are pre-normalised during index build
...
class Word2VecIndexer:
    """
    Trains a Word2Vec model on the corpus and builds per-document
    average-word-vector representations.
    """
    ...
    def __init__(self):
        self._model: Optional[object] = None   # gensim Word2Vec
        self._matrix: Optional[np.ndarray] = None  # (N, D) float32, L2-normalised
    ...
    def query_vector(self, tokens: list[str]) -> np.ndarray:
        """Return the L2-normalised average Word2Vec vector for query tokens."""
...
# ---------------------------------------------------------------------------
# BERT / Sentence-Transformer Indexer
# ---------------------------------------------------------------------------
class BERTIndexer:
    """
    Encodes all documents using a sentence-transformer model and stores
    the resulting embedding matrix.

    Default model: ``all-MiniLM-L6-v2`` (384-dim, fast, high quality).
    Swap for ``all-mpnet-base-v2`` (768-dim) for higher accuracy at the
    cost of ~3× more compute.
    """

    MODEL_NAME = "all-MiniLM-L6-v2"
    BATCH_SIZE = 256  # adjust to fit GPU/CPU RAM

    def __init__(self):
        self._model: Optional[object] = None  # SentenceTransformer
        self._matrix: Optional[np.ndarray] = None  # (N, 384) float32, L2-normalised
        self.doc_ids: list[str] = []
    ...
    def build(self, documents: list[dict]) -> None:
        if not _st_available:
            raise RuntimeError(
                "sentence-transformers is required for BERT indexing. "
                "pip install sentence-transformers"
            )
        logger.info(
            f"Encoding {len(documents)} documents with {self.MODEL_NAME}… "
            "(this may take several minutes on CPU)"
        )
        self.doc_ids = [d["doc_id"] for d in documents]
        # Use raw text if available, otherwise fall back to cleaned string
    ...
    def save(self, path: Path) -> None:
        path = Path(path)
        path.parent.mkdir(parents=True, exist_ok=True)
        # Save the model name (SentenceTransformer loads from HuggingFace hub)
