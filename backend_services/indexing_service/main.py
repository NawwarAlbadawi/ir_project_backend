...
from indexing_service.schemas import BuildIndexRequest, IndexStatusResponse, InvertedIndexQueryResponse, IndexStatsResponse
...
class _DatasetIndexState:
    ...
    def to_response(self) -> IndexStatusResponse:
        with self._lock:
            return IndexStatusResponse(dataset=self.name, status=self.status, built_models=list(self.built_models), progress=self.progress, total=self.total, error=self.error)
...
app = FastAPI(title='Indexing Service', description='Builds and exposes Inverted Index, TF-IDF, BM25, and Embedding indexes.', version='1.0.0', lifespan=lifespan)
...
def index_stats(dataset_name: str=Query(...)):
    ...
    return IndexStatsResponse(dataset=dataset_name, num_docs=state.inverted.N, vocab_size=state.inverted.vocab_size, available_models=state.built_models)

@app.get('/index/inverted', response_model=InvertedIndexQueryResponse, tags=['index'])
def query_inverted_index(dataset_name: str=Query(...), term: str=Query(..., description='The (preprocessed) term to look up.')):
    state = _get_or_create(dataset_name)
    ...
    return InvertedIndexQueryResponse(term=term, postings=info['postings'], df=info['df'])
...
def _fetch_all_documents(state: _DatasetIndexState, dataset_name: str) -> list[dict]:
    ...
    with httpx.Client(timeout=120.0) as client:
        status_resp = client.get(f'{PREPROCESSING_SERVICE_URL}/dataset/status', params={'dataset_name': dataset_name})
        status_resp.raise_for_status()
        total = status_resp.json().get('total_docs', 0)
        with state._lock:
            state.total = total
        while True:
            resp = client.get(f'{PREPROCESSING_SERVICE_URL}/dataset/docs', params={'dataset_name': dataset_name, 'offset': offset, 'limit': PAGE_SIZE})
