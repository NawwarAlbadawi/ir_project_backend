...
class SearchRequest(BaseModel):
    ...
    hybrid_weights: dict[str, float] = Field(
        default={"tfidf": 0.3, "bm25": 0.4, "bert": 0.3}
    )
...
class SearchResultItem(BaseModel):
    ...
    doc_id: str
    score: float
    snippet: Optional[str] = None   # Raw text snippet for display
...
class EvalCompareRequest(BaseModel):
    dataset: str
    model: str
    num_queries: int = Field(100, ge=1, le=5000, description="Number of queries to evaluate.")
    k: int = Field(10)
    compare_refinement: bool = Field(
        True,
        description="If True, evaluate both with and without refinement.",
    )
