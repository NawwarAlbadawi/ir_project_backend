...
app = FastAPI(title='Retrieval Service', description='Dispatches queries to TF-IDF, BM25, embedding, or hybrid retrievers.', version='1.0.0', lifespan=lifespan)
...
def list_models():
    return {'models': ['tfidf', 'bm25', 'word2vec', 'bert', 'hybrid_serial', 'hybrid_parallel']}

@app.get('/retrieve/datasets', tags=['retrieval'])
def list_datasets():
    with _lock:
        return {'datasets': [{'name': name, 'loaded_models': idx.loaded_models} for name, idx in _indexes.items()]}
...
def retrieve(request: RetrievalRequest):
    ...
    if idx is None:
        raise HTTPException(404, f"No indexes loaded for dataset '{request.dataset}'. Build indexes first via the Indexing Service.")
    t0 = time.perf_counter()
    results_raw: list[tuple[str, float]] = _dispatch(request, idx)
    latency_ms = (time.perf_counter() - t0) * 1000
    results = [RetrievalResult(doc_id=doc_id, score=score, rank=rank) for rank, (doc_id, score) in enumerate(results_raw[:request.top_k], start=1)]
    return RetrievalResponse(query_raw=request.query_raw, query_cleaned=request.query_cleaned, dataset=request.dataset, model=request.model, results=results, latency_ms=round(latency_ms, 2))

def _dispatch(req: RetrievalRequest, idx: _DatasetIndexes) -> list[tuple[str, float]]:
    model = req.model
    if model == 'tfidf':
        ...
        return idx.bm25.search(req.query_tokens, top_k=req.top_k, k1=req.bm25_k1, b=req.bm25_b)
    elif model == 'word2vec':
        ...
        return serial_hybrid(query_tokens=req.query_tokens, query_raw=req.query_raw or req.query_cleaned, bm25_indexer=idx.bm25, bert_indexer=idx.bert, top_k=req.top_k, candidate_k=req.serial_candidate_k, bm25_k1=req.bm25_k1, bm25_b=req.bm25_b)
    elif model == 'hybrid_parallel':
        return parallel_hybrid(query_tokens=req.query_tokens, query_cleaned=req.query_cleaned, query_raw=req.query_raw or req.query_cleaned, tfidf_indexer=idx.tfidf, bm25_indexer=idx.bm25, bert_indexer=idx.bert, top_k=req.top_k, fusion_method=req.fusion_method, weights=req.hybrid_weights, bm25_k1=req.bm25_k1, bm25_b=req.bm25_b)
    else:
